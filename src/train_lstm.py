...
from utils import (
    ...
    deconvolve,
    AudioLatentDataset,
    AudioInheritedLatentDataset,
    LatentDataset,
    CustomAudioImagePairing,
    InheritedLatentDataset,
    extract_latent_codes,
    extract_audio_latent_codes,
    extract_audio_latent_codes_vae,
)
...
def train_audio_lstm_vae(model_name, latents, size, num_epochs=100):
    ...
    if answer == "y":
        ...
        extract_audio_latent_codes_vae(
            model_name, t_latents, b_latents, size, output_path
        )
    ...
    lstm = AudioLatentLSTM(
        vocab_size,
        model.num_embeddings,
        hidden_dim=3,
        layers=3,
        audio_dim=audio_info.shape[1],
        audio_embed_dim=512,
    ).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        try:
            torch.save(
                lstm.state_dict(), join(root, "models", "LSTM", output_path, "lstm.pth")
            )
        except:
...
def train_audio_lstm_hierarchical(
    ...
    bottom_lstm = AudioInheritedLatentLSTM(
        vocab_size,
        model.num_embeddings,
        hidden_dim=4,
        layers=3,
    ...
    for epoch in range(num_epochs):
        for i, (top_seq, bottom_inputs, bottom_targets, audio) in enumerate(
            b_dataloader
        ):
        ...
        if epoch % 100 == 0 and epoch > 0:
            try:
                torch.save(
                    ...
                    join(
                        root, "models", "LSTM", output_path, f"BACKUP{epoch}_b_lstm.pth"
                    ),
                )
            except:
...
def train_vanilla_lstm_hierarchical(
    model_name, t_latents, b_latents, num_epochs=100, load_top=False
):
    ...
    lstm = LatentLSTM(vocab_size, model.num_embeddings, hidden_dim=3, layers=3).to(
        device
    )
    bottom_lstm = InheritedLatentLSTM(
        vocab_size, model.num_embeddings, hidden_dim=3, layers=3
    ).to(device)
    criterion = nn.CrossEntropyLoss()
    optimiser = optim.Adam(lstm.parameters(), lr=1e-3)
    ...
    if not load_top:
        for epoch in range(num_epochs):
            ...
            try:
                torch.save(
                    lstm.state_dict(),
                    join(root, "models", "LSTM", output_path, "t_lstm.pth"),
                )
            except:
                print("Couldn't save top LSTM")
    ...
    for epoch in range(num_epochs):
        ...
        try:
            torch.save(
                bottom_lstm.state_dict(),
                join(root, "models", "LSTM", output_path, "b_lstm.pth"),
            )
        except:
...
def decode(
    image_model, stored_location, t_lstm, b_lstm, image_size, size=128, use_audio=False
):
    ...
    try:
        model.load_state_dict(
            torch.load(join(root, "models", image_model), map_location=device)
        )
    except Exception as e:
        try:
            checkpoint = torch.load(
                join(root, "models", image_model), map_location=device
            )
            model.load_state_dict(checkpoint["vqgan"])
        except Exception as e:
            print(f"Unable to load model: {e}. Exiting...")
    ...
    try:
        if use_audio:
            ...
            top_lstm = LatentLSTM(
                model.num_embeddings, model.num_embeddings, hidden_dim=4, layers=3
            ).to(device)
            bottom_lstm = InheritedLatentLSTM(
                model.num_embeddings, model.num_embeddings, hidden_dim=4, layers=3
            ).to(device)

        top_lstm.load_state_dict(
            torch.load(
                join(root, "models/LSTM", stored_location, t_lstm), map_location=device
            )
        )
        bottom_lstm.load_state_dict(
            torch.load(
                join(root, "models/LSTM", stored_location, b_lstm), map_location=device
            )
        )
    ...
    if use_audio:
        ...
        AudioDataset = CustomAudioImagePairing(
            join(root, "data/downloaded_images"), join(root, "data/spectrograms"), size
        )

        for i in range(len(AudioDataset)):
            audio_input, _ = AudioDataset[i]
            audio_input = audio_input.unsqueeze(0).to(device)
            top_latents, bottom_latents = sample_latents_with_audio(
                top_lstm,
                bottom_lstm,
                0,
                image_size,
                "VQGAN-FT128/t_latents.pt",
                audio_input,
            )
            ...
            with torch.no_grad():
                ...
                Image.fromarray(
                    (
                        deconvolve(recon[0].cpu().detach().numpy().squeeze()).transpose(
                            1, 2, 0
                        )
                        * 255
                    ).astype(np.uint8)
                ).save(f"{root}/data/test_images/lstm_test_{i}.jpeg")

    else:
        top_latents, bottom_latents = sample_latents(
            top_lstm, bottom_lstm, 0, image_size * image_size
        )
        top_latents = top_latents.view(1, 32, 32)
        bottom_latents = bottom_latents.view(1, 64, 64)
        with torch.no_grad():
            ...
            ).save(f"{root}/data/test_images/lstm_test.jpeg")
...
def sample_latents(lstm, bottom_lstm, start_token, _, temperature=0.5):
    ...
    top_generated = (
        torch.tensor(top_generated, device=device).unsqueeze(0).long()
    )  # shape: (1, sequence_length)
    bottom_generated = []
    input_seq = torch.zeros(
        (1, 1), device=device
    ).long()  # Start with some neutral token
    hidden = None
...
def sample_latents_with_audio(
    lstm, bottom_lstm, start_token, size, t_latents, audio_input, temperature=0.5
):
...
if __name__ == "__main__":
    ...
    while True:
        answer = input(
            "Would you like to Extract Latents (1) or Train the LSTM (2), or Decode (3)? > "
        )
        if answer == "1":
            ...
            while True:
                ...
                if model_type == "1":
                    ...
                    train_audio_lstm_hierarchical(
                        model_name, t_latents, b_latents, size, num_epochs, Load
                    )
                    break
                elif model_type == "3":
                    train_vanilla_lstm_hierarchical(
