...
from models.vqvae import VQVAE, LatentLSTM, InheritedLatentLSTM
...
def train_lstm(num_epochs=100):
    ...
    try:
        top_latents = torch.load("top_latents.pt")
        bottom_latents = torch.load("bottom_latents.pt")
    ...
    try:
        torch.save(lstm.state_dict(), "t_lstm.pth")
        torch.save(bottom_lstm.state_dict(), "b_lstm.pth")
    except:
        print("\nFailed to save vae")
...
def extract_latent_codes():
    ...
    torch.save(stored_latent_t, "top_latents.pt")
    torch.save(stored_latent_b, "bottom_latents.pt")
